...
Never modern objects in frame.""",
    # ⑲ historydawn — 여명 복원 (2026-08-22 신설). historywarm 과 **질감·재질·금지는 같고
    #    색과 빛만 다르다.** 사용자가 가져온 샘플 5장(모아이·스톤헨지 편, 전부 docu3d)의
    #    문법을 실측해 조준했다: 차가운 하늘 + 지평선 온기 한 줄 + 낮게 깔린 물안개 + 역광.
    #    3차 조준 실측치 (사막신전/마을) — 채도 46~55·R−B −12~−16·한색 42~49%.
    #    목표였던 스톤헨지05 (채도 57·R−B −11·한색 36%) 에 거의 붙었다.
    #    1차 '거의 무채색'은 채도 16 흑백이 됐고, '밝게' 판은 여명이 사라져 낮이 됐다.
    #    온기의 세기는 톤이 아니라 weather_en 뒤 조각(빛)이 정한다 — 그래서 톤은 회청 바탕만 잡는다.
    "historydawn": """Style: rendered as a full-colour historical reconstruction filmed at first
light — the past rebuilt clean and whole, the way a museum reconstruction team would model it,
then filmed like a documentary. Materials keep that reconstruction character: ivory plaster,
pale weathered stone, honey-coloured timber, terracotta tile — clean freshly rebuilt surfaces
with real texture, no vintage film grain, no sepia wash, no scratches or old-photograph
artifacts.
The grade runs cool but never blue: the sky, the mist and every shadow sit in a faint grey-blue
so subtle it reads as absence of warmth rather than as a colour. The ONLY warmth in the frame is
a narrow band of low sun along the horizon and the rim light it throws — never a warm cast over
the whole picture.
...
STYLE_LABELS = {"docu3d": "다큐 3D (브랜드)", "techcool": "무광 저채도 (기술)",
                ...
                "techteal": "청록 무드 (기술)",
                "historywarm": "시대 재현 (역사)", "historydawn": "여명 복원 (역사)",
...
STYLE_GROUPS = [("3D (주력)", ["docu3d", "techcool", "techteal", "historywarm", "historydawn",
                               "tech3d", "sci3d", "arch3d"]),
...
It stands on the SAME ground line as the subject, close beside it, and the camera is placed so
both are seen from the same eye level — never one nearer and smaller, never one seen from above
while the other is seen from the side. Their heights must be directly comparable at a glance.
Its height relative to the subject matches the real measurements stated above: measure it against
the subject rather than guessing from what looks balanced in the frame.
The count stated above is exact — no more, no fewer. If several are needed, they line up in a neat
row, evenly spaced and all the same size.
...
ANNO_AUTO_STYLES = {"tech3d", "blueprint", "xsection", "sci3d", "docu3d",
                    "techcool", "techteal", "historywarm", "historydawn",   # 신톤 — 레퍼런스가 강조 다용 톤
...
· **짝 톤 historydawn(여명 복원)** — **질감·재질·금지사항은 historywarm 과 똑같고 색과 빛만
  다르다.** 회청 하늘·안개·그림자에 지평선 온기가 한 줄, 대상은 역광 실루엣, 물안개가 낮게
  깔린다. **훅·여운·비장한 장면**에 쓰고, 밝은 낮에 사건이 벌어지는 설명 컷은 historywarm 이다.
  한 편에서 훅=historydawn → 본론=historywarm 흐름이 자연스럽다.
  온기의 세기는 톤이 아니라 weather_en 뒤 조각(빛)이 정한다
  (약하게: "warm low dawn light" / 세게: "a narrow band of deep amber sun right on the horizon").
...
· scale  — **크기가 안 와닿을 때** 익숙한 것과 나란히 놓는다. compare_en 필수.
           ...
           **compare_en 은 「개수 + 대상 + 실제 치수」 세 조각으로 써라** — 앱이 그 숫자로
           크기를 맞춘다. 개수를 안 쓰면 몇 개가 나올지 모르고, 치수를 안 쓰면 눈대중이 된다.
             나쁨: cargo trucks parked in a row
             좋음: exactly three city buses in a row, each 12 m long and 3 m tall
           **모델이 실제 크기를 아는 것만 골라라** — 사람(1.7m) · 승용차(4.5m) · 시내버스(12m) ·
           문(2m) · 농구 골대(3m) · 2층 주택(7m). 코끼리·고래·공룡은 개체차가 커서 부정확하다.
...
{{
  ...
  "cuts": [
    {{
      ...
      "style": "docu3d | techcool | techteal | historywarm | historydawn | bonebody | bodyviz | tech3d | sci3d | arch3d | snap | cine | archive | illust | labmacro | blueprint | aerial | xsection | claysection | planline | tabletop | blackstage | game | story3d | toy3d | greycast | whitecast | anime — **이 목록에 없는 값을 지어내지 마라. 앱이 조용히 snap(폰카 스냅샷)으로 바꿔 톤이 통째로 어긋난다.** 목록에 없는 느낌이 필요하면 가장 가까운 값을 골라라 — 미니어처·축소 세계는 tabletop, 무채색 단면 모형은 claysection, 테이블 위 실물 시연은 labmacro",
